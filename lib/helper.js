...
function _string(str, escape = false) {
  if (escape) {
    // Emit path: AST already holds unescaped content; encode for Go literals.
    return _goEscape(str.string);
  }
  // Read path: keep historical quote-stripping behaviour for callers that
  // inspect Tea/Dara string tokens without emitting Go source.
  if (str.string === '""') {
    return '""';
  }
  return str.string.replace(/([^\\])"+|^"/g, function (matched) {
