...
/**
 * Escape a raw string so it is safe inside a Go double-quoted literal.
 * Backslashes must be escaped before quotes, otherwise `\` + `"` becomes invalid Go.
 *
 * @param {string} str
 * @param {{ keepNewlines?: boolean }} [options]
 *   keepNewlines: leave real `\n` in the result so template_string can still
 *   split with `replace(/[\n]/g, '" + \\n"')` (legacy emit style / runtime).
 */
function _goEscape(str, options = {}) {
  let out = str
    .replace(/\\/g, '\\\\')
    .replace(/"/g, '\\"')
    .replace(/\0/g, '\\x00')
...
function _string(str, escape = false, options) {
  if (escape) {
    // Emit path: AST already holds unescaped content; encode for Go literals.
    return _goEscape(str.string, options);
  }
  // Read path: AST `.string` is already the raw content; no quote rewrite.
  // (Previously a replace(/"/g, '"') no-op — removed per review.)
  return str.string;
}
...
function _getAttributes(node, attrName) {
  for (let i = 0; i < node.attrs.length; i++) {
    if (_name(node.attrs[i].attrName) === attrName) {
...
module.exports = {
  _name, _string, _goEscape, _type, _format, _initValue, _avoidReserveName, _importFilter, _upperFirst, _snakeCase,
  _setExtendFunc, _isFilterType, _getAttr, _getAttributes, _setValueFunc, _vid, _pointerType, _lowerFirst, _escape, _deleteWithSuffix,
