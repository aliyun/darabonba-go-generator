...
class Stream extends Builtin {
  ...
  // dara.ReadAsSSE takes io.ReadCloser, but `$readable()` maps to dara.ToReader
  // (io.Reader): emit an assert-and-fallback so the underlying HTTP body stays
  // closable instead of a non-compiling direct call.
  readAsSSE(args, level, env, argHasThrowFunc) {
    const g = this.generator;
    const source = args.find(a => !a.yieldArg);
    const yieldArg = args.find(a => a.yieldArg);
    const saved = g.output;
    g.output = '';
    g.visitExpr(source, level, env);
    const srcExpr = g.output;
    g.output = saved;
    // io.NopCloser is Go 1.16+; ioutil.NopCloser keeps generated SDKs
    // building on the openapi Go CI matrix (1.13–1.15).
    g.imports.push({ pkgName: 'io' });
    g.imports.push({ pkgName: 'io/ioutil' });
    g.emit(`sseBody, _ok := ${srcExpr}.(io.ReadCloser)\n`);
    g.emit(`if !_ok {\n`, level);
    g.emit(`sseBody = ioutil.NopCloser(${srcExpr})\n`, level + 1);
    g.emit(`}\n`, level);
    g.emit(`dara.ReadAsSSE(sseBody, `, level);
    if (yieldArg) {
      g.visitYieldArgs(yieldArg.ast, level, env);
    }
    g.emit(`)`);
  }
}
