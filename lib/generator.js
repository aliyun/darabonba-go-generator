...
const {
  _name, _format, _string, _type, _initValue, _structType, _avoidReserveName, 
...
class Visitor {
  ...
  visitModule(ast, level) {
    ...
    if (ast.imports.length > 0) {
      ...
      for (let i = 0; i < ast.imports.length; i++) {
        ...
        const aliasId = item.lexeme.replace(/-/g, '_');
  ...
  visitParams(ast, level, env) {
    ...
    for (var i = 0; i < ast.params.length; i++) {
      ...
      env.local.set(name, node.paramType);
    }
  ...
  visitRuntimeBefore(ast, level, env) {
    ...
    if (_name(env.returnType) && _name(env.returnType) !== 'void') {
      ...
      this.visitPointerType(env.returnType);
      this.emit(`)\n`);
    }
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        ...
        if (node.fieldValue.fieldItemType.lexeme) {
          this.emit(`[]${_structType(node.fieldValue.fieldItemType.lexeme)} `);
        ...
        if (fieldType.type === 'moduleModel'){
          this.visitType(fieldType, level);
          this.emit(` `);
        } else if (!_name(fieldType)) {
          if (fieldType === 'map') {
            this.visitType(node.fieldValue, level);
            this.emit(` `);
          } else {
            this.emit(`${_structType(fieldType)} `);
          }
        } else if (_name(fieldType) && fieldType.idType === 'module') {
          this.emit(`${this.clientName[_name(fieldType)]} `);
        } else {
          this.emit(`${_structType(_name(fieldType))} `);
        }
      }
  ...
  checkAnnotation(node, attrName){
    for (let i = 0; i < node.attrs.length; i++) {
      if (attrName === node.attrs[i].attrName.lexeme) {
        return true;
      }
    }
    return false;
  ...
  parseAnnotation(node, annos){
    var tag = '';
    for (let i = 0; i < node.attrs.length; i++) {
      const attrValueType = annos[node.attrs[i].attrName.lexeme];
      if (attrValueType) {
        var attrName = node.attrs[i].attrName.lexeme;
  ...
  addSetFunc(ast, structName, level) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        if (node.fieldValue.fieldItemType.lexeme) {
          this.emit(`func (s *${structName}) Set${fieldName}(v []${_structType(node.fieldValue.fieldItemType.lexeme)}) *${structName} {\n`, level);
          ...
          this.emit(`func (s *${structName}) Set${fieldName}(v []${_structType(fileldtype)}) *${structName} {\n`, level);
          this.emit(`s.${fieldName} = v\n`, level + 1);
          this.emit(`return s\n`, level + 1);
          this.emit(`}\n`, level);
          this.emit(`\n`, level);
  ...
  visitPropertyAccess(ast, level, env, expected) {
    ...
    if (id === '__module') {
      ...
      for (let i = 0; i < ast.propertyPath.length; i++) {
        value = value[_name(ast.propertyPath[i])];
      }
      this.emit(`"${value}"`);
      return;
    }
  ...
  visitExpr(ast, level, env, expected) {
    ...
    if (ast.type === 'boolean') {
      ...
      if (!ast.inferred) {
        this.emit(`nil`);
      } else if (_name(ast.inferred)) {
        var name = '';
        if (ast.inferred.moduleName) {
          name = ast.inferred.moduleName.toLowerCase() + '.' + _name(ast.inferred);
        } else {
          name = _name(ast.inferred);
        }
      ...
      for (let i = 0; i < ast.elements.length; i++) {
        ...
        if (item.type === 'element') {
          ...
          if (expr.type === 'property_access' && _name(expr.id) === '__module') {
            var value = this.__module;
            for (let i = 0; i < expr.propertyPath.length; i++) {
              value = value[_name(expr.propertyPath[i])];
            }
            this.emit(`"${value}"`);
          } else if(expr.type === 'property_access' && 
          expr.propertyPathTypes[expr.propertyPathTypes.length-1].name === 'string') {
            this.visitExpr(expr, level, env);
          } else {
  ...
  visitReturn(ast, level, env) {
    ...
    if (ast.expr.type === 'call') {
      var hasThrow = false;
      if (ast.expr.vid) {
        hasThrow = env.predefined[_name(ast.expr.vid)].value.hasThrow;
      } else {
  ...
  visitAssign(ast, level, env) {
    if (ast.left.type === 'id') {
      ...
      this.emit(``, level);
      this.visitPropertyAccess(ast.left, level, env, {needCast: 'false'});
    ...
    if(ast.expr.needToReadable){
      ...
      this.emit(`)`);
    } else if (ast.expr.type === 'call' && ast.expr.id && ast.expr.id.inferred.type === 'model') {
      this.visitExpr(ast.expr, level, env, {needCast: 'false', type: 'pointer'});
      this.emit(``);
  ...
  visitDeclare(ast, level, env) {
    ...
    var type = this.getExprType(ast.expr, env);
    env.local.set(id, type);
  ...
  getVariableType(id, env) {
    return env.local.get(id.lexeme).lexeme;
  }

  getExprType(ast, env) {
    if (ast.type === 'property_access') {
      if (!ast.propertyPathTypes) {
        return ast.id.inferred.type;
      }
      const types = ast.propertyPathTypes;
      ...
      if (ast.id.lexeme === '__response') {
        return '$Response';
      }
      return this.getVariableType(ast.id, env);
    } else if (ast.type === 'virtualVariable') {
      const type = env.types.get(ast.vid.lexeme);
      if (type) {
        return this.getType(type.value);
      }
      // 未定义 虚拟变量 默认 string 类型
      return 'string';
    } else if (ast.type === 'template_string') {
      return 'string';
    }
