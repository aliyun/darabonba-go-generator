...
class Visitor {
    ...
    if (ast.catchBlocks && ast.catchBlocks.length > 0) {
      ast.catchBlocks.forEach(catchBlock => {
        ...
        if (!catchBlock.id.type) {
          this.emit(`if _t, ok := _err.(dara.BaseError); ok {\n`, level + 1);
        } else {
  ...
  visitSetFunc(ast, structName, level = 0){
    ...
    if (ast.fieldValue.fieldType === 'array') {
      if (_name(ast.fieldValue.fieldItemType)) {
        this.emit(`func (s *${structName}) Set${fieldName}(v []${_pointerType(_name(ast.fieldValue.fieldItemType))}) *${structName} {\n`, level);
  ...
  visitExceptionInterfceBody(ast, structName, level) {
    assert.equal(ast.type, 'exceptionBody');
    for (let i = 0; i < ast.nodes.length; i++) {
      const node = ast.nodes[i];
  ...
  visitModelInterfaceBody(ast, structName, level) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        if (_name(node.fieldValue.fieldItemType)) {
          this.emit(`Set${fieldName}(v []${_pointerType(_name(node.fieldValue.fieldItemType))}) *${structName}\n`, level);
        } else if (node.fieldValue.fieldItemType.type === 'map') {
          this.emit(`Set${fieldName}(v []`, level);
          this.visitType(node.fieldValue.fieldItemType);
          this.emit(`) *${structName}\n`);
        } else if (node.fieldValue.fieldItemType.type === 'modelBody') {
          this.emit(`Set${fieldName}(v []${_pointerType(fileldtype)}) *${structName}\n`, level);
        } else if (node.fieldValue.fieldItemType.fieldType === 'array') {
          this.emit(`Set${fieldName}(v [][]`, level);
  ...
  visitReturn(ast, level, env) {
    ...
    if(ast.expr.type === 'null') {
      if(!env.finallyBlock) {
        if (env.hasThrow) {
          this.emit(`return nil , nil`, level);
        } else {
          this.emit(`return nil`, level);
        }
        this.emit(`\n`);
      }
      return;
    ...
    if (ast.expr.type === 'variable' ||
      ...
      if(!env.finallyBlock) {
        if (env.hasThrow) {
          this.emit(`return _result , nil`, level);
        } else {
  ...
  visitAssign(ast, level, env) {
    ...
    if (hasThrowCall) {
      this.emit(`, _err ${(newVar || env.yieldFunc) ? ':' : ''}= `);
    } else {
      this.emit(` = `);
    }
  ...
  eachAPI(ast, level, predefined) {
    ...
    if (ast.runtimeBody) {
      this.emit('}\n', level + 1);
      this.emit(`return nil, _resultErr\n`, level + 1);
    }
