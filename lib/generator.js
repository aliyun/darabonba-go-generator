...
class Visitor {
  ...
  visitImports(imports, usedTypes, innerModule) {
    ...
    if (imports.length > 0) {
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        if (innerPath) {
          ...
          this.clientName[aliasId] = '*' + _importFilter(_format(aliasId).toLowerCase()) + '.' + this.constructFunc[aliasId];
    ...
    if (usedTypes.has('readable') || usedTypes.has('writeable')) {
      if (this.goPackages !== '') {
        this.goPackages += `\n`;
      }
      this.goPackages += `  "io"`;
  ...
  visitReturnType(ast, level, env, needClose = true) {
    if (_name(ast.returnType) !== 'void') {
      const isIterator = _isIterator(ast.returnType);
      this.emit(` (_result ${isIterator ? '<-chan ' : ''}`);
  ...
  getExprVars(ast) {
    ...
    if (ast.type === 'property_access') {
      ...
      vars = vars.concat(this.getObjectVars(ast.expr));
  ...
  visitAPIBody(ast, apiName, level, env) {
    ...
    if(errCounts > 1) {
      const funcName = `${_lowerFirst(apiName)}_opRequest`;
      ...
      this.emit(`request_, _err = ${funcName}`, level);
      this.visitTryArgs(args);
    ...
    if (ast.type === 'return') {
      ...
      if (ast.type === 'call' && ast.hasThrow) {
        ...
        if(env.runtimeBody){
          ...
          this.visitCatch(env.try, level, env);
        } else {
  ...
  addSetFunc(ast, structName, level = 0) {
  ...
  visitExtendOn(extendOn, type = 'model', level) {
    if(!extendOn) {
    ...
    if (extendOn.type === 'moduleModel') {
      ...
      const [moduleId, ...rest] = extendOn.path;
      this.emit(`${_format(_name(moduleId))}`, level);
      this.emit(rest.map((item) => {
        return _format(_name(item));
      }).join(''));
    } else {
      ...
      this.emit(`${_format(_name(extendOn))}`);
    }
    this.emit('\n');
  }
  ...
  visitException(ast, level) {
    ...
    this.visitExtendOn(ast.extendOn, 'exception', level + 1);
  ...
  visitModel(ast, level) {
    ...
    this.visitExtendOn(ast.extendOn, 'model', level + 1);
  ...
  visitExpr(ast, level, env, expected, argHasThrowFunc) {
    ...
    if (ast.type === 'boolean') {
      ...
      isPointer = false;
      var id = _name(ast.id);
      if (!env.pointerParams.includes(id) && (expected && expected.pointer) && (ast.inferred.type !== 'model' && 
      ...
      if(env.pointerParams.includes(id) && (!expected || !expected.pointer) && 
      ((ast.inferred.type === 'array' && ast.inferred.itemType.type === 'basic') || 
      (DSL.util.isBasicType(_name(ast.inferred)) && !_isFilterType(_name(ast.inferred))))) {
        isPointer = true;
      ...
      //   this.emit(`${_setValueFunc(_name(ast.inferred))}`);
      //   this.visitCall(ast, level, env, argHasThrowFunc);
      //   if(_name(ast.inferred) !== 'void') {
      //     this.emit(')');
      //   }
      // } else {
      //   if (expected && expected.pointer && !this.isNotBuiltin(ast) && _name(ast.inferred) !== 'void') {
      //     this.emit(`${_setExtendFunc(_name(ast.inferred))}`);
      //   }
  ...
  visitConstruct(ast, level, env) {
    ...
    if(moudleName.startsWith('$') && this.builtin[moudleName]) {
      ...
      moudleName = _type(moudleName);
    } else {
      moudleName = _format(moudleName).toLowerCase();
    }
    this.emit(`${moudleName}.New${constructFunc}(`);
    for (let i = 0; i < ast.args.length; i++) {
  ...
  visitConstructModel(ast, level, env) {
    ...
    if ((ast.inferred && ast.inferred.moduleName) || ast.aliasId.isModule) {
      ...
      const modelName = _format(_name(ast.aliasId));
      this.emit(`&${_modelName(modelName)}`);
    }
    ...
    if (ast.object && ast.object.fields.length > 0) {
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        if ((_name(field.expectedType) === 'number' || _name(field.expectedType) === 'integer') && _name(field.expr.inferred) === 'int32') {
          ...
          this.visitExpr(field.expr, level + 1, env, expected);
        }
        if (str !== '') {
          this.emit(`)`);
        }
  ...
  visitThrow(ast, level, env) {
    if (ast.expr.type === 'construct_model') {
      ...
      this.emit(`_err ${env.yieldFunc ? ':' : ''}= dara.New${CORE}Error(`, level);
  ...
  visitAssign(ast, level, env) {
    ...
    if (ast.left.type === 'property_assign' || ast.left.type === 'property') {
      ...
      pointer = env.pointerParams.includes(name);
    ...
    if (ast.expr.needToReadable) {
      ...
      if ((ast.left.inferred.name === 'number' || ast.left.inferred.name === 'integer') && ast.expr.inferred.name === 'int32') {
        this.emit(`${pointer ? 'tea.ToInt(' : 'int('}`);
        ...
        this.emit(`${pointer ? 'tea.ToInt32(' : 'int32('}`);
        this.visitExpr(ast.expr, level, env, { pointer });
        this.emit(`)`);
      } else {
        this.visitExpr(ast.expr, level, env, { pointer });
  ...
  getVarDealFunc(ast, expected) {
    ...
    if(expected && expected.pointer && !this.isNotBuiltin(ast)) {
  ...
  visitDeclare(ast, level, env) {
    ...
    if (ast.expectedType) {
      if ((_name(ast.expectedType) === 'number' || _name(ast.expectedType) === 'integer') && expr.inferred.name === 'int32') {
        ...
        this.emit(`)`);
      } else if (expr.type !== 'number' && (expr.inferred.name === 'number' || expr.inferred.name === 'integer') && _name(ast.expectedType) === 'int32') {
        this.emit(`int32(`);
        this.visitExpr(expr, level, env, expected, argHasThrowFunc);
        this.emit(`)`);
      } else {
  ...
  visitYieldReturnBody(ast, funcName, level, env) {
    ...
    this.emit('_yield := make(chan ', level);
    this.visitPointerType(env.returnType.valueType);
    this.emit(`)\n`);
    if(env.hasThrow) {
    ...
    this.emit(`go ${funcName}(_yield`, level);
    if(env.hasThrow) {
      this.emit(', _yieldErr, ');
    }
    ...
    params.map((name) => {
      const variable = env.pointerParams.includes(name) ? false : true;
      args.push({
        name,
        type: argsMap[name],
        variable,
  ...
  visitYieldFunction(ast, level, env) {
    ...
    const pointerParams = [];
    this.emit('_yield := make(chan ', level);
    this.visitPointerType(ast.returnType.valueType);
    this.emit(`)\n`);
    args.push({
      name: '_yield chan<-',
    ...
    this.emit(`go ${functionName}(_yield`, level);
    if(env.hasThrow) {
      this.emit(', _yieldErr');
    }
  ...
  visitAPIErrCatch(level, env) {
    ...
    if(env.runtimeBody) {
      ...
      this.emit('retryPolicyContext = &dara.RetryPolicyContext{\n', level + 1);
      this.emit('RetriesAttempted: retriesAttempted,\n', level + 2);
      this.emit('Request:          request_,\n', level + 2);
      this.emit('Response:         response_,\n', level + 2);
  ...
  eachAPI(ast, level, predefined) {
    ...
    // temp level
    this.visitAPIBody(ast.apiBody, apiName, baseLevel + 1, env);
  ...
  visitTryParams(args, env) {
    ...
    params.map((param, index) => {
      this.emit(`${param} `);
      if(env.pointerParams.includes(param)){
