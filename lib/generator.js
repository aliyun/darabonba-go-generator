...
class Visitor {
  ...
  visitModule(ast, level) {
    ...
    const cons = ast.moduleBody.nodes.filter((item) => {
    ...
    for (let i = 0; i < cons.length; i++) {
      const con = cons[i];
      this.visitAnnotation(con.annotation, level);
      this.emit(`const ${_name(con.constName)} = `, level);
      if (con.constValue.string) {
        this.emit(`"${con.constValue.string}"\n`);
      } else {
        this.emit(`${con.constValue.value}\n`);
      }
    }
  ...
  visitAssign(ast, level, env) {
    if (ast.left.type === 'property_assign' || ast.left.type === 'property') {
      ...
      this.emit(``, level);
      this.visitMapAccess(ast.left, level, env, {type: 'pointer'});
    } else {
      throw new Error('unimpelemented');
    }
