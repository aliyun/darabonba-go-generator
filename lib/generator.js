...
class Visitor {
  ...
  visitMethodCall(ast, level, env, argHasThrowFunc) {
    ...
    if (name.startsWith('$') && this.builtin[name]) {
      ...
      this.builtin[name][method](ast.args, level, env);
      return;
    } 
    if (!ast.isStatic) {
      this.emit(`client.`);
  ...
  visitBuiltinStaticCall(ast, level, env) {
    ...
    builtiner[func](ast.args, level, env);
  }
  ...
  functionBodyRetrun(stmt, env) {
    ...
    // console.log(this.functionBodyRetrun(stmt.tryBlock.stmts[stmt.tryBlock.stmts.length - 1]));
