...
class Visitor {
      ...
      for (let i = 0; i < ast.imports.length; i++) {
        ...
        if (importTeaFile.go && importTeaFile.go.clientName) {
          if (importTeaFile.go.clientName.indexOf('*')===0) {
            ...
            this.clientName[aliasId] = '*' + _format(aliasId).toLowerCase() + '.' + importTeaFile.go.clientName.substring(1);
          } else {
            this.constructFunc[aliasId] = importTeaFile.go.clientName;
            this.clientName[aliasId] = _format(aliasId).toLowerCase() + '.' + importTeaFile.go.clientName;
          }
        } else {
          this.constructFunc[aliasId] = 'Client';
          this.clientName[aliasId] = '*' + _format(aliasId).toLowerCase() + '.Client';
        }
        if (!goPkg) {
        ...
        this.importPackages += `  ${_format(aliasId).toLowerCase()}  "${pkgName}"\n`;
      }
    }
  ...
  visitTry(ast, level, env) {
    ...
    if (tryBlock && tryBlock.stmts.length > 0) {
      ...
      if (catchBlock && catchBlock.stmts.length > 0) {
        ...
        this.emit(`} else {\n`, level + 1);
        this.emit(`${_name(ast.catchId)}.SetErrMsg(tryErr.Error())\n`, level + 2);
        this.emit(`}\n`, level + 1);
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        ...
        if (_name(node.fieldValue.fieldItemType)) {
          ...
          this.emit(`[][]`);
          this.visitPointerType(node.fieldValue.fieldItemType.fieldItemType);
          this.emit(` `);
        }
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        if (_name(node.fieldValue.fieldItemType)) {
          ...
          this.emit(`func (s *${structName}) Set${fieldName}(v [][]` +
            `${_pointerType(_name(node.fieldValue.fieldItemType.fieldItemType))}) *${structName} {\n`, level);
          this.emit(`s.${fieldName} = v\n`, level + 1);
          this.emit(`return s\n`, level + 1);
          this.emit(`}\n`, level);
          this.emit(`\n`, level);
  ...
  visitExpr(ast, level, env, expected) {
    ...
    if (ast.type === 'boolean') {
      ...
      if (expected && expected.pointer) {
        if (expected.name) {
          this.emit(`${_setExtendFunc(expected.name)}${ast.value.value})`);
        } else {
  ...
  visitAssign(ast, level, env) {
    ...
    if(ast.expr.needToReadable){
     ...
     && ast.expr.type !== 'virtualVariable' && ast.expr.type !== 'variable') {
