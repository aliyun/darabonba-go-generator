...
const {
  ...
  _setExtendFunc, _isFilterType, _getAttr, _getAttributes, _setValueFunc, _vid, _pointerType, _lowerFirst, _escape,
...
class Visitor {
  constructor(option = {}) {
    ...
    this.defaultReturn = this.config.defaultReturn;
  ...
  visitModule(ast, filepath, main, level) {
    ...
    this.visitContextFunc(ast, apis, funcs, packageName, filepath.replace('.go', '_context_func.go'));
    this.saveInnerModule(ast);
  }

  visitContextFunc(ast, apis, funcs, packageName, filepath) {
    this.ctxFunc = {};
    this.output = '';
    this.imports = [];
    this.builtinModule = [];
    ...
    let hasCtx = false;
    for (let i = 0; i < apis.length; i++) {
      const context = apis[i].notes && apis[i].notes.find(note => note.note.lexeme === '@context');
      if(context && context.arg.value) {
        this.eachAPI(apis[i], level, ast.predefined, _string(context.arg.value));
        hasCtx = true;
      }
    }
    ...
    for (let i = 0; i < funcs.length; i++) {
      const context = funcs[i].notes && funcs[i].notes.find(note => note.note.lexeme === '@context');
      if(context && context.arg.value && funcs[i].isAsync) {
        this.eachFunc(funcs[i], level, ast.predefined, apis, _string(context.arg.value));
        hasCtx = true;
      }
    }
    if(!hasCtx) {
    ...
    for (let i = 0; i < this.tryFunc.length; i++) {
      this.eachTryFunc(this.tryFunc[i], level, true);
    }

    for (let i = 0; i < this.yieldFunc.length; i++) {
      this.eachYieldFunc(this.yieldFunc[i], level, true);
    }
  ...
  visitParams(ast, level, env) {
    ...
    this.emit('(');
    if(env.isCtxFunc) {
      this.emit(`ctx context.Context${ast.params.length > 0 ? ', ' : ''}`);
    }
  ...
  getObjectVars(ast, env) {
    ...
    for(let i = 0; i < fields.length; i++) {
      vars = vars.concat(this.getExprVars(fields[i].expr, env));
    }
    for(let i = 0; i < expandFields.length; i++) {
      vars = vars.concat(this.getExprVars(expandFields[i].expr, env));
    }
    return vars;
  }
  ...
  getExprVars(ast, env) {
    ...
    if (ast.type === 'property_access') {
      ...
      vars.concat(this.getObjectVars(ast, env));
      ...
      vars = vars.concat(this.getExprVars(ast.expr, env));
      ...
      for (let i = 0; i < ast.elements.length; i++) {
        ...
        if (item.type === 'expr') {
          ...
          vars = vars.concat(this.getExprVars(expr, env));
        }
      }
    } else if (ast.type === 'call') {
      if((ast.left.type === 'instance_call' || ast.left.type === 'method_call') && 
        env.isCtxFunc && ast.notes && ast.notes.find(note => note.note.lexeme === '@context')) {
        vars.push({
          name: 'ctx',
      ...
      if(ast.left.type === 'instance_call') {
        if (_name(ast.left.id).indexOf('@') === 0) {
          ...
          vars.push({
            name: _avoidReserveName(_name(ast.left.id)),
            type: type
          });
        }
      ...
      vars = vars.concat(this.getExprVars(ast.left, env));
      vars = vars.concat(this.getExprVars(ast.right, env));
    } else if (ast.type === 'construct') {
      for (let i = 0; i < ast.args.length; i++) {
        vars = vars.concat(this.getExprVars(ast.args[i], env));
      }
    } else if (ast.type === 'construct_model' && ast.object) {
      vars = vars.concat(this.getObjectVars(ast.object, env));
    } else if (ast.type === 'not') {
      vars = vars.concat(this.getExprVars(ast.expr, env));
    } else if (ast.type === 'array') {
      for (let i = 0; i < ast.items.length; i++) {
        vars = vars.concat(this.getExprVars(ast.items[i], env));
      }
  ...
  getStmtsVars(stmts, env) {
    let args = [];
    let declare = [];
    for (var i = 0; i < stmts.length; i++) {
      const ast = stmts[i];
      if (ast.type === 'return') {
        args = args.concat(this.getExprVars(ast.expr, env));
      } else if (ast.type === 'if') {
        args = args.concat(this.getExprVars(ast.condition, env));
        args = args.concat(this.getStmtsVars(ast.stmts.stmts, env));
        if(ast.elseIfs && ast.elseIfs.length > 0) {
          ast.elseIfs.map(elseIf => {
            args = args.concat(this.getExprVars(elseIf.condition, env));
            args = args.concat(this.getStmtsVars(elseIf.stmts.stmts, env));
          });
        }
        if(ast.elseStmts) {
          args = args.concat(this.getStmtsVars(ast.elseStmts.stmts, env));
        }
      } else if (ast.type === 'throw') {
        args = args.concat(this.getExprVars(ast.expr, env));
      } else if (ast.type === 'assign') {
        args = args.concat(this.getExprVars(ast.left, env));
        args = args.concat(this.getExprVars(ast.expr, env));
      } else if (ast.type === 'declare') {
        declare.push(_avoidReserveName(_name(ast.id)));
        args = args.concat(this.getExprVars(ast.expr, env));
      } else if (ast.type === 'while') {
        args = args.concat(this.getExprVars(ast.condition, env));
        args = args.concat(this.getStmtsVars(ast.stmts.stmts, env));
      } else if (ast.type === 'for') {
        args = args.concat(this.getExprVars(ast.list, env));
        args = args.concat(this.getStmtsVars(ast.stmts.stmts, env));
      } else if (ast.type === 'try') {
        args = args.concat(this.getStmtsVars(ast.stmts.stmts, env));
      } else if (ast.type === 'call') {
        args = args.concat(this.getExprVars(ast, env));
      }  else {
        args = args.concat(this.getExprVars(ast, env));
      }
    }
  ...
  visitStmt(ast, level, env) {
    ...
    if (ast.type === 'return') {
      ...
      if(_isIterator(ast.inferred)) {
        ...
        const validate = ast.left.type === 'instance_call' && builtinInstance && 
          ast.builtinModule === '$ModelInstance' && _name(ast.left.propertyPath[0]) === 'validate';
        if(validate && !this.noCompatible && !env.isStatic) {
          this.emit(`if dara.BoolValue(client.EnableValidate) == true {\n`, level++);
        }
        if (ast.inferred && _name(ast.inferred) !== 'void') {
          this.emit(`_, _err ${(env.yieldFunc && !env.runtimeBody) ? ':' : ''}= `, level);
        } else {
          this.emit(`_err ${(env.yieldFunc && !env.runtimeBody) ? ':' : ''}= `, level);
        }
        this.visitExpr(ast, level, env, { pointer: false });
        this.emit(`\n`);
        if(env.runtimeBody){
  ...
  visitTry(ast, level, env) {
    ...
    if(errCounts > 1) {
      ...
      const args = this.getStmtsVars(tryBlock.stmts, env);
  ...
  visitType(ast, level) {
    if (ast.type === 'moduleModel') {
      ...
      this.emit(`*`);
      if (ast.moduleName) {
        const { pkgName } = this.module.importPackages[ast.moduleName];
        this.imports.push({
          aliasId: ast.moduleName,
          pkgName,
        });
  ...
  visitValidate(nodes, structName, level = 0) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      const node = nodes[i];
      if (this.fieldNeedsValidation(node)) {
        hasValidation = true;
        break;
      }
    }
    ...
    if (!hasValidation) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        const node = nodes[i];
        this.visitFieldValidate(node, level + 1, structName);
      }
      this.emit(`return nil\n`, level + 1);
    }
  ...
  fieldNeedsValidation(node) {
    const pattern = _getAttributes(node, 'pattern') || '';
    const maxLength = parseInt(_getAttributes(node, 'maxLength')) || 0;
    const minLength = parseInt(_getAttributes(node, 'minLength')) || 0;
    const maximum = parseFloat(_getAttributes(node, 'maximum')) || 0;
    const minimum = parseFloat(_getAttributes(node, 'minimum'));
    const requireAttr = _getAttributes(node, 'require');
    const required = node.required || requireAttr === 'true' || requireAttr === true;
    
    // 获取字段类型，用于判断验证规则的适用性
    const fieldType = node.fieldValue.fieldType;
    const fieldTypeName = fieldType ? _name(fieldType) : '';
    const isNumericType = fieldType === 'number' || fieldType === 'integer' || 
                         fieldTypeName === 'number' || fieldTypeName === 'integer' ||
                         fieldTypeName === 'int' || fieldTypeName === 'long' ||
                         fieldTypeName === 'double' || fieldTypeName === 'float';
    
    // 包含必需字段验证和显式验证属性
    const hasFieldValidation = required || maxLength > 0 || minLength > 0 || 
                               (maximum > 0 && isNumericType) || 
                               (!isNaN(minimum) && minimum >= 0 && isNumericType) || 
                               pattern !== '';
    
    const hasNestedValidation = this.fieldHasNestedValidation(node);
    
    return hasFieldValidation || hasNestedValidation;
  }

  fieldHasNestedValidation(node) {
    const fieldValue = node.fieldValue;
    
    // 检查数组类型
    if (fieldValue.fieldType === 'array') {
      return fieldValue.fieldItemType && (
        fieldValue.fieldItemType.type === 'modelBody' ||
        fieldValue.fieldItemType.type === 'moduleModel' ||
        fieldValue.fieldItemType.type === 'subModel' ||
        fieldValue.fieldItemType.idType === 'model'
      );
    }
    // 检查map类型
    else if (fieldValue.type === 'map') {
      return fieldValue.valueType && (
        fieldValue.valueType.type === 'modelBody' ||
        fieldValue.valueType.type === 'moduleModel' ||
        fieldValue.valueType.type === 'subModel' ||
        fieldValue.valueType.idType === 'model'
      );
    }
    // 检查单个结构体类型
    else if (fieldValue.type === 'modelBody' || 
             (fieldValue.fieldType && (
               fieldValue.fieldType.type === 'moduleModel' ||
               fieldValue.fieldType.type === 'subModel' ||
               fieldValue.fieldType.idType === 'model'
             ))) {
      return true;
    }
  ...
  visitFieldValidate(node, level, structName) {
    const attrName = _name(node.fieldName);
    const fieldName = _format(attrName);
    const pattern = _getAttributes(node, 'pattern') || '';
    const maxLength = parseInt(_getAttributes(node, 'maxLength')) || 0;
    const minLength = parseInt(_getAttributes(node, 'minLength')) || 0;
    const maximum = parseFloat(_getAttributes(node, 'maximum')) || 0;
    ...
    const fieldType = node.fieldValue.fieldType;
    const fieldTypeName = fieldType ? _name(fieldType) : '';
    const isStringType = fieldType === 'string' || fieldTypeName === 'string';
    const isNumericType = fieldType === 'number' || fieldType === 'integer' || 
                         fieldTypeName === 'number' || fieldTypeName === 'integer' ||
                         fieldTypeName === 'int' || fieldTypeName === 'long' ||
                         fieldTypeName === 'double' || fieldTypeName === 'float';
    
    if (required || maxLength > 0 || minLength > 0 || (maximum > 0 && isNumericType) || 
        (!isNaN(minimum) && minimum >= 0 && isNumericType) || pattern !== '') {
      
      if (required) {
        this.emit(`if err := dara.ValidateRequired(s.${fieldName}, "${fieldName}"); err != nil {\n`, level);
        this.emit(`return err\n`, level + 1);
        this.emit(`}\n`, level);
      }
      
      const needsNullCheck = !required && (pattern !== '' || maxLength > 0 || minLength > 0 || 
                                          (maximum > 0 && isNumericType) || 
                                          (!isNaN(minimum) && minimum >= 0 && isNumericType));
      if (needsNullCheck) {
        this.emit(`if s.${fieldName} != nil {\n`, level);
        level += 1;
      }
      ...
      if (isStringType) {
        if (pattern !== '') {
          this.emit(`if err := dara.ValidatePattern(s.${fieldName}, \`${pattern}\`, "${fieldName}"); err != nil {\n`, level);
          this.emit(`return err\n`, level + 1);
          this.emit(`}\n`, level);
        }
        
        if (maxLength > 0 && maxLength <= 2147483647) {
          this.emit(`if err := dara.ValidateMaxLength(s.${fieldName}, ${maxLength}, "${fieldName}"); err != nil {\n`, level);
          this.emit(`return err\n`, level + 1);
          this.emit(`}\n`, level);
        }
        
        if (minLength > 0 && minLength <= 2147483647) {
          this.emit(`if err := dara.ValidateMinLength(s.${fieldName}, ${minLength}, "${fieldName}"); err != nil {\n`, level);
          this.emit(`return err\n`, level + 1);
          this.emit(`}\n`, level);
        }
      }
      ...
      if (isNumericType) {
        if (maximum > 0 && maximum <= Number.MAX_SAFE_INTEGER) {
          this.emit(`if err := dara.ValidateMaximum(s.${fieldName}, ${maximum}.0, "${fieldName}"); err != nil {\n`, level);
          this.emit(`return err\n`, level + 1);
          this.emit(`}\n`, level);
        }
        
        if (!isNaN(minimum) && minimum >= 0 && minimum <= Number.MAX_SAFE_INTEGER) {
          this.emit(`if err := dara.ValidateMinimum(s.${fieldName}, ${minimum}.0, "${fieldName}"); err != nil {\n`, level);
          this.emit(`return err\n`, level + 1);
          this.emit(`}\n`, level);
        }
      }
    ...
    this.visitNestedFieldValidate(node, level, fieldName);
  }

  visitNestedFieldValidate(node, level, fieldName) {
    const fieldValue = node.fieldValue;
    
    if (fieldValue.fieldType === 'array') {
      // 只在数组元素需要验证时才生成循环代码
      if (fieldValue.fieldItemType && (fieldValue.fieldItemType.type === 'modelBody' ||
          fieldValue.fieldItemType.type === 'moduleModel' ||
          fieldValue.fieldItemType.type === 'subModel' ||
          (fieldValue.fieldItemType.idType === 'model'))) {
        this.emit(`if s.${fieldName} != nil {\n`, level);
        this.emit(`for _, item := range s.${fieldName} {\n`, level + 1);
        this.emit(`if item != nil {\n`, level + 2);
        this.emit(`if err := item.Validate(); err != nil {\n`, level + 3);
        this.emit(`return err\n`, level + 4);
        this.emit(`}\n`, level + 3);
        this.emit(`}\n`, level + 2);
        this.emit(`}\n`, level + 1);
      ...
      // 只在map值需要验证时才生成循环代码
      if (fieldValue.valueType && (fieldValue.valueType.type === 'modelBody' ||
          fieldValue.valueType.type === 'moduleModel' ||
          fieldValue.valueType.type === 'subModel' ||
          (fieldValue.valueType.idType === 'model'))) {
        this.emit(`if s.${fieldName} != nil {\n`, level);
        this.emit(`for _, value := range s.${fieldName} {\n`, level + 1);
        this.emit(`if value != nil {\n`, level + 2);
        this.emit(`if err := value.Validate(); err != nil {\n`, level + 3);
        this.emit(`return err\n`, level + 4);
        this.emit(`}\n`, level + 3);
        this.emit(`}\n`, level + 2);
        this.emit(`}\n`, level + 1);
      ...
      this.emit(`if s.${fieldName} != nil {\n`, level);
      this.emit(`if err := s.${fieldName}.Validate(); err != nil {\n`, level + 1);
      this.emit(`return err\n`, level + 2);
      this.emit(`}\n`, level + 1);
      this.emit(`}\n`, level);
    }
  ...
  visitObject(ast, level, env, expected) {
    ...
    if (ast.fields.length === 0) {
      ...
      if (!hasExpandField) {
        ...
        const beginNotes = DSL.note.getNotes(this.notes, ast.tokenRange[0], ast.tokenRange[1]);
        const part = beginNotes.find(note => note.note.lexeme === '@go');
        if(part && part.arg.value) {
          this.emit(part.arg.value.string, level + 1);
        }
  ...
  visitArgs(args, level, env, expected, argHasThrowFunc) {
    this.emit('(');
    if(env.isCtxFunc && expected && expected.useCtx) {
      this.emit(`ctx${args.length > 0 ? ', ' : ''}`);
    }
    let hasYieldArg = false;
    args = args.filter(arg => {
      if(arg.yieldArg && !hasYieldArg) {
        hasYieldArg = true;
        return true;
      }
      if(arg.yieldArg && hasYieldArg) {
  ...
  visitMethodCall(ast, level, env, argHasThrowFunc) {
    ...
    const useCtx = env.isCtxFunc && ast.notes && ast.notes.find(note => note.note.lexeme === '@context');
    const sse = ast.notes && ast.notes.find(note => note.note.lexeme === '@sse');
    const isSSE = sse && sse.arg.value;
    const name = useCtx ? _string(useCtx.arg.value) : _name(ast.left.id);
    if (name.startsWith('$') && this.builtin[name]) {
    ...
    this.emit(_format(name));
    this.visitArgs(ast.args, level, env, { pointer: true, useCtx }, argHasThrowFunc);
  }
  ...
  visitInstanceCall(ast, level, env, argHasThrowFunc) {
    ...
    const sse = ast.notes && ast.notes.find(note => note.note.lexeme === '@sse');
    const isSSE = sse && sse.arg.value;
    const method = useCtx ? _string(useCtx.arg.value) : _name(ast.left.propertyPath[0]);
    const builtinInstance = ast.builtinModule && this.builtin[ast.builtinModule];
    ...
    if (_name(ast.left.id).indexOf('@') === 0) {
      ...
      this.emit(`${_name(ast.left.id)}.${useCtx ? _format(_string(useCtx.arg.value)) : _format(method)}`);
    }
    this.visitArgs(ast.args, level, env, { pointer: builtinInstance ? false : true, useCtx }, argHasThrowFunc);
  }
  ...
  visitExpr(ast, level, env, expected, argHasThrowFunc) {
    ...
    if (ast.type === 'boolean') {
      ...
      // Escape once for Go: do not combine unescaped _string() with an extra
      // .replace(/"/g, '\\"'), which used to turn Dara "a\"b" into invalid "a\\"b".
      if (expected && expected.pointer) {
        this.emit(`dara.String("${_string(ast.value, true)}")`);
      } else {
        this.emit(`"${_string(ast.value, true)}"`);
      }
      ...
      isPointer = false;
      var id = _avoidReserveName(_name(ast.id));
      ...
      for (let i = 0; i < ast.elements.length; i++) {
        ...
        if (item.type === 'element') {
          // Keep real newlines so the legacy `"..." + \n "..."` split still
          // matches pre-fix template emit (avoids turning multiline templates
          // into a single literal with `\n`, which changed runtime content).
          let val = _string(item.value, true, { keepNewlines: true });
          val = val.replace(/[\n]/g, '" + \n"');
          this.emit(`"${val}"`);
  ...
  visitYieldArgs(ast, level, env) {
    ...
    if (ast.left.type === 'property_assign' || ast.left.type === 'property') {
      ...
      const name = _avoidReserveName(_name(ast.left.id));
  ...
  visitReturn(ast, level, env) {
    ...
    if(!this.exec && this.defaultReturn) {
      let returnType = '';
      if (env.returnType.idType === 'module') {
  ...
  visitThrow(ast, level, env) {
    ...
    if(env.try) {
      ...
      if(env.yieldFunc) {
        if(env.runtimeBody) {
          // inline iterator returns-block: thrown errors feed the retry catch
          this.visitAPIErrCatch(level, env);
        } else {
          this.emit(`_yieldErr <- _err\n`, level);
          this.emit(`return\n`, level);
  ...
  visitYieldReturnBody(ast, funcName, level, env) {
    // Inline the returns-block statements into the API main flow (no _opResponse
    // helper): spi/gateway errors feed the retry catch directly, same as
    // non-iterator APIs. The trailing `return null` is dropped and replaced by
    // the final-error drain + retry catch + return.
    const stmts = ast.stmts.stmts;
    const last = stmts.length > 0 ? stmts[stmts.length - 1] : null;
    const dropTailReturn = last && last.type === 'return' &&
      (!last.expr || last.expr.type === 'null');
    if (dropTailReturn) {
      this.visitStmts({ ...ast.stmts, stmts: stmts.slice(0, -1) }, level, env);
    } else {
      this.visitStmts(ast.stmts, level, env);
    }
  ...
  visitReturnBody(ast, apiName, level, env) {
    ...
    if(errCounts > 1) {
      const args = this.getStmtsVars(ast.stmts.stmts, env);
      this.tryFunc.push({
        args,
  ...
  visitYieldFunction(ast, level, env) {
    ...
    this.emit('', level);
    if(ast.isStatic === false) {
      this.emit('client.');
    }
    this.emit(`${functionName}(_yield`);
    if(env.hasThrow) {
      this.emit(', _yieldErr');
    }
    if(env.isCtxFunc) {
  ...
  eachFunc(ast, level, predefined, apis, ctxFunc = false) {
    const env = {
      predefined,
      apis,
      isCtxFunc: !!ctxFunc,
    ...
    const functionName = ctxFunc ? ctxFunc : _name(ast.functionName);
    if (ctxFunc) {
      this.ctxFunc[_name(ast.functionName)] = ctxFunc;
    }
    this.visitAnnotation(ast.annotation, level);
    let comments = DSL.comment.getFrontComments(this.comments, ast.tokenRange[0]);
    this.visitComments(comments, level);
  ...
  eachAPI(ast, level, predefined, ctxFunc = false) {
    // if (ast.annotation) {
    ...
    const apiName = ctxFunc || _name(ast.apiName);
    if(ctxFunc) {
      this.ctxFunc[_name(ast.apiName)] = apiName;
    }
    this.visitAnnotation(ast.annotation, level);
    let comments = DSL.comment.getFrontComments(this.comments, ast.tokenRange[0]);
    this.visitComments(comments, level);
    ...
    // this.emit(`${RESPONSE}, _err := `, baseLevel + 1);

    // if (ast.runtimeBody) {
    //   this.emit(`func() (*dara.Response, error) {\n`);
    //   this.emit(`if _runtime.Ctx == nil {\n`, baseLevel + 2);
    //   this.emit(`return dara.DoRequest(request_, _runtime)\n`, baseLevel + 3);
    //   this.emit(`}\n`, baseLevel + 2);
    //   this.emit(`return dara.DoRequestWithCtx(_runtime.Ctx, request_, _runtime)\n`, baseLevel + 2);
    //   this.emit(`}()\n`, baseLevel + 1);
    // } else {
    //   this.emit(`dara.DoRequest(${REQUEST}, nil)\n`);
    // }

    if (ast.runtimeBody) {
      this.emit(`${RESPONSE}, _err = dara.DoRequest${ctxFunc ? 'WithCtx(ctx, ' : '('}${REQUEST}`, baseLevel + 1);
    } else {
      this.emit(`${RESPONSE}, _err := dara.DoRequest${ctxFunc ? 'WithCtx(ctx, ' : '('}${REQUEST}`, baseLevel + 1);
    }    
  ...
  visitHeader(__module, packageName, level) {
    ...
    if (/\bdara\.[A-Za-z_]\w*/.test(this.output)) {
  ...
  moduleAfter() {
    ...
    this.header = this.header.replace(/import \(\n\)\n\n/g, '');
  }
  ...
  eachYieldFunc(ast, level, isCtxFunc) {
    const env = {
      local: new Map(),
      returnType: {
        name: 'void'
    ...
    if (ast.isStatic === false) {
      this.emit(`func (client *${this.structName}) ${ast.name}(`, level);
    } else {
  ...
  eachTryFunc(ast, level, isCtxFunc) {
    const env = {
      local: new Map(),
      returnType: ast.returnType,
      hasThrow: true,
