...
const {
  _name, _format, _string, _type, _initValue, _avoidReserveName, _importFilter, _upperFirst,
  _setExtendFunc, _isFilterType, _getAttr, _setValueFunc, _vid, _pointerType, _lowerFirst
} = require('./helper');
...
class Visitor {
  ...
  visitModule(ast, level) {
    ...
    if (this.config.go && this.config.go.interface) {
      ...
      for (let func of NonStaticFuncs) {
        ...
        this.emit(`${_name(func.functionName)} `, level + 1);
  ...
  visitModelBody(ast, lastName, level) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      for (let i = 0; i < node.attrs.length; i++) {
        if (_name(node.attrs[i].attrName) === 'description') {
          const tag = node.attrs[i].attrValue['string'];
    ...
    if (ast.fields.length === 0) {
      ...
      if (!hasExpandField) {
        ...
        for (i = 0; i < ast.fields.length; i++) {
          this.visitObjectField(ast.fields[i], level + 1, env, { pointer: fieldType });
        }
  ...
  visitStaticCall(ast, level, env, argHasThrowFunc) {
    ...
    this.emit(`${_format(_name(ast.left.id)).toLowerCase()}.${_format(_name(ast.left.propertyPath[0]))}(`);
    for (let i = 0; i < ast.args.length; i++) {
      const expr = ast.args[i];
      if (expr.needCast) {
        this.emit('tea.ToMap(');
  ...
  visitMapAccess(ast, level, env, expected) {
    ...
    if ((!expected || !expected.pointer) && (DSL.util.isBasicType(_name(ast.inferred)) &&
      ...
      this.emit(`${mapName}[`);
      this.visitExpr({ ...ast.accessKey, inferred: ast.inferred }, level, env);
      this.emit(`]`);
    }
  }
  ...
  visitArray(ast, level, env, expected) {
    ...
    for (let i = 0; i < ast.items.length; i++) {
      ...
      this.visitExpr(item, level + 1, env, { pointer: true });
  ...
  visitAssign(ast, level, env) {
    let importPkg = this.module.importPackages.find(item => {
      return item.name === ast.left.inferred.name;
    });
    let isInterfaceModel = false;
    if (importPkg && importPkg.goPkg.interface) {
    ...
    if (ast.left.type === 'property_assign' || ast.left.type === 'property') {
      ...
      if (isInterfaceModel && ast.expr.type === 'construct') {
        this.emit(`interface${_name(ast.left.inferred)}`, level);
      } else {
    ...
    if (hasThrowCall) {
      this.emit(`, _err ${isInterfaceModel && ast.expr.type === 'construct' ? ':' : ''}= `);
    } else {
      this.emit(` ${isInterfaceModel && ast.expr.type === 'construct' ? ':' : ''}= `);
    }
    if (ast.expr.needToReadable) {
      ...
      if ((ast.left.inferred.name === 'number' || ast.left.inferred.name === 'integer') && ast.expr.inferred.name === 'int32') {
        ...
        this.visitExpr(ast.expr, level, env, { pointer: true });
        this.emit(`)`);
      } else if (isInterfaceModel && ast.expr.type !== 'construct') {
        this.emit(`interface${_name(ast.left.inferred)}.Client`, level);
      } else {
        this.visitExpr(ast.expr, level, env, { pointer: true });
      }
    }
