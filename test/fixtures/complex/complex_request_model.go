...
func (s *ComplexRequest) Validate() error {
  ...
  if s.FloatList != nil {
    for _, item := range s.FloatList {
    }
  }
  if err := dara.ValidateRequired(s.BooleantList, "BooleantList"); err != nil {
    return err
  }
  if s.BooleantList != nil {
    for _, item := range s.BooleantList {
    }
  }
